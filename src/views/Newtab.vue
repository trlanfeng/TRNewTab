<template>
    <div>
        ...
        <div class="search_box" v-show="userdata.isSearchOpen">
            ...
            <input type="text" @keyup.enter="search" class="form-control" v-model="keywords" :placeholder="'通过 '+ userdata.searchTitle +' 来搜索'">
            <button class="search_button" @click="search">搜索</button>
        </div>
        <div id="SpeedDialContainer">
            <draggable v-model="userdata.list" :options="draggableOptions" class="SpeedDialBox" @end="moveItem">
                <div v-for="(item,index) in userdata.list" :key="index" class="speeddial col-12 col-sm-4 col-md-3 col-lg-3">
                    ...
                    <div @click="removeItem(index)" v-if="isEditMode" class="close_button"><img src="../assets/images/delete.png"></div>
                </div>
            </draggable>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="tab-pane" v-show="settingIndex==0">
                            ...
                            <div class="networkImage" v-show="userdata.bgType==2">
                                <div class="form-group">
                                    <label>网络图片地址：</label>
                                    <input type="text" class="form-control" v-model="userdata.bgUrl"
                            ...
                            <div class="blur_range">
                                <div class="form-group">
                                    ...
                                    <input type="range" min="0" max="100" step="1" v-model="userdata.bgBlur" class="custom-range" id="customRange1">
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane" v-show="settingIndex==1">
                            <div class="form-group switcher_box">
                                <label>是否开启搜索：</label>
                                <toggle-button class="switcher" :speed="150" :height="32" :width="55" v-model="userdata.isSearchOpen"/>
                            </div>
                            <div class="search_settings">
                                <div class="form-group">
                                    <label>搜索名称：</label>
                                    <input type="text" class="form-control" v-model="userdata.searchTitle"
                                        placeholder="请填写搜索名称">
                                </div>
                                <div class="form-group">
                                    <label>搜索地址：</label>
                                    <input type="text" class="form-control" v-model="userdata.searchUrl"
                                     placeholder="请填写搜索地址（例如：https://www.baidu.com/s?wd=）">
                                </div>
                                <div class="form-group">
                                    <label>图标地址：</label>
                                    <input type="text" class="form-control" v-model="userdata.searchIcon"
